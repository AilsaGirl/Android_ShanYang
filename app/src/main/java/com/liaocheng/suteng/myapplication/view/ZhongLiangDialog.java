 ...
 * Descript:
 */
public class ZhongLiangDialog extends Dialog {
    ...
    public ZhongLiangDialog(@NonNull Context context) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dialog_zhongliang);
            ...
            public void onClick(View v) {
                if (yesOnclickListener != null) {
                    if (!TextUtils.isEmpty(etTel.getText().toString().trim())){
                        if (Integer.parseInt(etTel.getText().toString().trim())==0){
                            ToastUtil.show("重量不能为0");
                            return;
                        }
                    }else {
                        ToastUtil.show("重量不能为空");
                        return;
                    }
