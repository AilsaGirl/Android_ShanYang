...
public class FaHuoSongFaHuoActivity extends BaseActivity<FaHuoSongPresenter> implements FaHuoSongContact.View, TextWatcher, LocationSource,
...
boolean isSouSuo = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolBarl.setTitleText("发货地址").setLeftClick(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (isSouSuo){
                    ...
                    final ApplyAndAlterDialog dialog = new ApplyAndAlterDialog(FaHuoSongFaHuoActivity.this);
        ...
        tvCity.setText(addressModel.city);
    ...
    public void onCallWrapper() {
        ...
        for (int i = 0; i < permissions.length; i++) {
            if (ContextCompat.checkSelfPermission(FaHuoSongFaHuoActivity.this, permissions[i]) != PackageManager.PERMISSION_GRANTED) {
        ...
        if (mPermissionList.isEmpty()) {//未授予的权限为空，表示都授予了
            ...
            ActivityCompat.requestPermissions(FaHuoSongFaHuoActivity.this, permissions, REQUEST_CODE_ASK_CALL_PHONE);
        }
    }
        ...
        switch (requestCode) {
            case REQUEST_CODE_ASK_CALL_PHONE:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(FaHuoSongFaHuoActivity.this, "Permission Denied",
    ...
    private void init() {
        ...
        if (addressModel.city.contains("聊城")){
            setUpMap();
        }else {
            getLatlon(addressModel.city);
        }
//        getLatlon(addressModel.city);
//        setUpMap();
    ...
    protected void doSearchQuery(int type) {
        ...
        int currentPage = 0;
        //搜索（S：关键字，
        // S1：汽车服务|汽车销售|汽车维修|摩托车服务|餐饮服务|购物服务|生活服务|体育休闲服务|医疗保健服务|住宿服务|风景名胜|商务住宅|政府机构及社会团体|科教文化服务|交通设施服务|金融保险服务|公司企业|道路附属设施|地名地址信息|公共设施
        // S3：城市，可以空字符串，空字符串代表全国
        // 三个个都可以为空）

        if(type==1){
            query = new PoiSearch.Query("", "", addressModel.city+"");
            ...
            query = new PoiSearch.Query(deepType, "", addressModel.city+"");
            query.setPageSize(50);// 设置每页最多返回多少条poiitem
            query.setPageNum(currentPage);// 设置查第一页
            poiSearch = new PoiSearch(this, query);
            poiSearch.setOnPoiSearchListener(this);
